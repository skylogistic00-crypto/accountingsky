...
import { BarChart3, TrendingUp, Package, FileText, ShoppingCart, Users, DollarSign, ArrowUpRight, Plus, Receipt } from 'lucide-react';
...
type ViewType = 'overview' | 'requests' | 'suppliers' | 'users' | 'inventory' | 'create-request' | 'create-supplier' | 'permohonan-dana' | 'create-permohonan';
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-12">
            <div>
              ...
              <p className="text-slate-400">Welcome back, {userProfile?.full_name || 'Admin'}</p>
            </div>
          </div>
          ...
          {currentView === 'overview' ? (
            <>
              {/* 3D Cards Grid */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8" style={{ perspective: '2000px' }}>
                {/* Accounting Card */}
                <div 
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.transform = 'rotateY(-5deg) rotateX(0deg) translateZ(20px)';
                  }}
                  onMouseLeave={(e) => {
                ...
                >
                  <div 
                    ...
                    style={{
                      boxShadow: '30px 30px 80px rgba(0,0,0,0.5), -10px -10px 40px rgba(255,255,255,0.03)',
                      border: '1px solid rgba(255,255,255,0.05)'
                    }}
                  >
                    <h2 className="text-white text-3xl font-bold mb-8" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.3)' }}>
                      Accounting
                    </h2>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      >
                        <div className="text-white text-4xl font-bold mb-1">{permohonanDana.length}</div>
                        <div className="text-orange-100 text-sm font-medium">Payment</div>
                      </div>
                      <div 
                      ...
                      >
                        ...
                        <div className="text-teal-100 text-sm font-medium">Number</div>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-white text-2xl font-bold">8.4K</span>
                    ...
                    >
                      <h3 className="text-slate-800 font-bold text-lg mb-4">Permohonan Dana</h3>
                      <div className="space-y-3">
                        {permohonanDana.slice(0, 4).map((req, i) => (
                          <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                            <div>
                              <div className="text-slate-700 font-semibold text-sm">{req.nama_pemohon}</div>
                              <div className="text-slate-500 text-xs">{req.departemen}</div>
                            </div>
                            <span className={`${getStatusColor(req.status)} text-white px-3 py-1 rounded-full text-xs font-medium shadow-md`}>
                              {req.status}
                            </span>
                          </div>
                        ))}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <div className="text-teal-100 text-sm font-medium">Inventory Movement</div>
                        </div>
                        <span className="text-white text-3xl font-bold">8.4K</span>
                      </div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-slate-800 font-bold text-lg">Purchase Request / Order</h3>
                        <span className="text-orange-500 text-2xl font-bold">8.4K</span>
                      </div>
                      <div className="space-y-3">
                        {allRequests.slice(0, 4).map((req, i) => (
                          <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                            <div>
                              <div className="text-slate-700 font-semibold text-sm">{req.pr_po_number || req.request_number}</div>
                              <div className="text-slate-500 text-xs">{req.item_name}</div>
                            </div>
                            <span className={`${getStatusColor(req.status)} text-white px-3 py-1 rounded-full text-xs font-medium shadow-md`}>
                              {req.status}
                            </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                >
                  ...
                  <div className="text-white text-4xl font-bold mb-2">{stats.totalSuppliers}</div>
                  <div className="text-slate-400 text-sm font-medium">Total Suppliers</div>
                </div>
                ...
                >
                  ...
                  <div className="text-white text-4xl font-bold mb-2">{stats.totalUsers}</div>
                ...
                >
                  ...
                  <div className="text-white text-4xl font-bold mb-2">
                    Rp {allRequests.reduce((sum, req) => sum + (parseInt(req.unit_price) * req.qty || 0), 0).toLocaleString('id-ID')}
                  </div>
                ...
                >
                  ...
                  <div className="text-white text-4xl font-bold mb-2">Cash Book</div>
                  <div className="text-slate-400 text-sm font-medium">Penerimaan & Pengeluaran Kas</div>
                </div>
              </div>
            </>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">Form Permohonan Dana</h2>
                <button
                  onClick={() => setCurrentView('permohonan-dana')}
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-slate-800">Permohonan Dana</h2>
              ...
              <Table>
                ...
                <TableBody>
                  {permohonanDana.map((req, i) => (
                    <TableRow key={i}>
                      <TableCell className="font-medium">{req.nama_pemohon}</TableCell>
                      <TableCell>{req.departemen}</TableCell>
                      <TableCell>{new Date(req.tanggal_permohonan).toLocaleDateString('id-ID')}</TableCell>
                      <TableCell>Rp {parseFloat(req.jumlah).toLocaleString('id-ID')}</TableCell>
                      <TableCell>{req.keterangan || '-'}</TableCell>
                      <TableCell>
                        <span className={`${getStatusColor(req.status)} text-white px-3 py-1 rounded-full text-xs`}>
                          {req.status}
                        </span>
                      </TableCell>
                    </TableRow>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">Create Supplier</h2>
                <button
                  onClick={() => setCurrentView('suppliers')}
                  className="px-4 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-700 transition-colors"
                >
                  ...
                  <TableBody>
                    {allRequests.map((req, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>Rp {parseInt(req.unit_price).toLocaleString('id-ID')}</TableCell>
                        <TableCell>
                          <span className={`${getStatusColor(req.status)} text-white px-3 py-1 rounded-full text-xs`}>
                            {req.status}
                          </span>
                  ...
                  <TableBody>
                    {allSuppliers.map((supplier, i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell>
                          <span className={`${supplier.is_pkp ? 'bg-green-500' : 'bg-gray-500'} text-white px-3 py-1 rounded-full text-xs`}>
