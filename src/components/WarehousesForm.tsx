...
export default function WarehousesForm() {
  ...
  useEffect(() => {
    ...
    const warehouseChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'warehouses' }, fetchAllData)
      .subscribe();
    
    const zoneChannel = supabase
      .channel('zones-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'zones' }, fetchAllData)
      .subscribe();
    
    const rackChannel = supabase
      .channel('racks-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'racks' }, fetchAllData)
      .subscribe();
    
    const lotChannel = supabase
      .channel('lots-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'lots' }, fetchAllData)
      .subscribe();
  ...
  const fetchAllData = async () => {
    try {
      const [warehousesRes, zonesRes, racksRes, lotsRes] = await Promise.all([
        ...
        supabase.from('racks').select('*').order('created_at', { ascending: false }),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        if (error) throw error;
        toast({ title: 'Success', description: `${activeTab} created successfully` });
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (error) throw error;
      toast({ title: 'Success', description: `${activeTab} deleted successfully` });
    } catch (error: any) {
      toast({ title: 'Error', description: error.message, variant: 'destructive' });
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-100">Kelola gudang, zona, rak, dan lot</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <Card className="border-none shadow-lg bg-purple-400/90 text-white hover:shadow-xl transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardDescription className="text-white/90">Warehouses</CardDescription>
                <Building2 className="h-8 w-8 text-white/80" />
              </div>
              <CardTitle className="text-4xl font-bold">{summaryData.warehouses}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-none shadow-lg bg-emerald-400/90 text-white hover:shadow-xl transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardDescription className="text-white/90">Zones</CardDescription>
                <MapPin className="h-8 w-8 text-white/80" />
              </div>
              <CardTitle className="text-4xl font-bold">{summaryData.zones}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center text-sm text-white/90">
          ...
          <Card className="border-none shadow-lg bg-pink-400/90 text-white hover:shadow-xl transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardDescription className="text-white/90">Racks</CardDescription>
                <Grid3x3 className="h-8 w-8 text-white/80" />
              </div>
              <CardTitle className="text-4xl font-bold">{summaryData.racks}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center text-sm text-white/90">
          ...
          <Card className="border-none shadow-lg bg-blue-400/90 text-white hover:shadow-xl transition-shadow">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardDescription className="text-white/90">Lots</CardDescription>
                <Package2 className="h-8 w-8 text-white/80" />
              </div>
              <CardTitle className="text-4xl font-bold">{summaryData.lots}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center text-sm text-white/90">
                    ...
                    <TableBody>
                      {filteredWarehouses.map((warehouse) => (
                        ...
                        >
                          ...
                          <TableCell className="text-slate-700">{warehouse.code}</TableCell>
                          <TableCell className="text-slate-700">{warehouse.address || '-'}</TableCell>
                          <TableCell className="text-slate-700">{warehouse.city || '-'}</TableCell>
                          <TableCell>
                      ...
                      {filteredZones.map((zone) => {
                        ...
                        return (
                          <TableRow 
                            ...
                            className="hover:bg-indigo-50 transition-colors border-b border-slate-100"
                          >
                            <TableCell className="font-medium text-slate-900">{zone.name}</TableCell>
                            <TableCell className="text-slate-700">{zone.code}</TableCell>
                            <TableCell className="text-slate-700">{warehouse?.name || '-'}</TableCell>
                            <TableCell className="text-slate-700">{zone.description || '-'}</TableCell>
                            <TableCell>
                      ...
                      {filteredRacks.map((rack) => {
                        ...
                        return (
                          <TableRow 
                            ...
                            className="hover:bg-indigo-50 transition-colors border-b border-slate-100"
                          >
                            <TableCell className="font-medium text-slate-900">{rack.name}</TableCell>
                            <TableCell className="text-slate-700">{rack.code}</TableCell>
                            <TableCell className="text-slate-700">{zone?.name || '-'}</TableCell>
                            <TableCell className="text-slate-700">{rack.level || '-'}</TableCell>
                            <TableCell className="text-slate-700">{rack.capacity || '-'}</TableCell>
                            <TableCell>
                      ...
                      {filteredLots.map((lot) => {
                        ...
                        return (
                          <TableRow 
                            ...
                            className="hover:bg-indigo-50 transition-colors border-b border-slate-100"
                          >
                            <TableCell className="font-medium text-slate-900">{lot.lot_number}</TableCell>
                            <TableCell className="text-slate-700">{lot.item_name || '-'}</TableCell>
                            <TableCell className="text-slate-700">{rack?.name || '-'}</TableCell>
                            <TableCell className="text-slate-700">{lot.quantity}</TableCell>
                            <TableCell className="text-slate-700">
                              {lot.manufacturing_date ? new Date(lot.manufacturing_date).toLocaleDateString('id-ID') : '-'}
                            </TableCell>
                            <TableCell className="text-slate-700">
                              {lot.expiry_date ? new Date(lot.expiry_date).toLocaleDateString('id-ID') : '-'}
                            </TableCell>
                            <TableCell>
            ...
            <>
              {activeTab === 'warehouses' && filteredWarehouses.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium text-lg">Tidak ada warehouses ditemukan</p>
              ...
              {activeTab === 'zones' && filteredZones.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium text-lg">Tidak ada zones ditemukan</p>
                  <p className="text-sm text-slate-400 mt-1">Coba ubah filter atau pencarian</p>
                </div>
              )}
              {activeTab === 'racks' && filteredRacks.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium text-lg">Tidak ada racks ditemukan</p>
                  <p className="text-sm text-slate-400 mt-1">Coba ubah filter atau pencarian</p>
                </div>
              )}
              {activeTab === 'lots' && filteredLots.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  <p className="text-slate-500 font-medium text-lg">Tidak ada lots ditemukan</p>
                  <p className="text-sm text-slate-400 mt-1">Coba ubah filter atau pencarian</p>
                </div>
              )}
            </>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem ? `Update the ${activeTab} information` : `Create a new ${activeTab}`}
            </DialogDescription>
          </DialogHeader>
                ...
                <>
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setFormData({ ...formData, rack_id: value })}
                    >
                      <SelectTrigger>
